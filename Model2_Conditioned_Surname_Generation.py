...
nationality_vocab = vectorizer.nationality_vocab

# 关键修改2: 指定要生成的国家名称
try:
    target_nationality = "French"  # 需要生成的国籍名称
    nationality_index = nationality_vocab.lookup_token(target_nationality)
except KeyError:
    print(f"错误：数据集中没有'{target_nationality}'标签！")
    print("请检查数据集中存在的国家标签：", list(nationality_vocab._token_to_idx.keys()))
    exit()

# 关键修改3: 生成指定国家的姓氏
num_samples = 5  # 生成数量
temperature = 0.7  # 控制生成随机性（0.1-1.5之间调整）

sampled_indices = sample_from_model(
    model,
    vectorizer,
    nationalities=[nationality_index] * num_samples,  # 重复索引生成多个样本
    sample_size=20,    # 最大生成长度
    temperature=temperature
)

# 解码并打印结果
print(f"\n生成的 {target_nationality} 姓氏示例（温度={temperature}）：")
for i, name in enumerate(decode_samples(sampled_indices, vectorizer)):
    print(f"{i+1}. {name}")